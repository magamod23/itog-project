...
class gg:
    def __init__(self):
        # создаем соединение с gg
        self.conn = sqlite3.connect('db.db')
        # создание объекта класса cursor, используемый для взаимодействия с gg
        self.c = self.conn.cursor()
        # выполнение запроса к gg
        self.c.execute(
            '''CREATE TABLE IF NOT EXISTS db (id integer primary key, name text, tel text, email text, salary text)''')
        # сохранение изменений gg
        self.conn.commit()

    # метод добавления в gg
